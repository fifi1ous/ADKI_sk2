...
void Algorithms::analyzeSlope(const std::vector<Edge> &dt, std::vector<Triangle> &triangles, const bool &click)
{

    //Analyze DTM slope
    if (triangles.size()==0 || click )
...
void Algorithms::analyzeAspect(const std::vector<Edge> &dt, std::vector<Triangle> &triangles, const bool &click)
{
    //Analyze DTM Aspect
    if (triangles.size()==0 || click)
        edgesToTriangle(dt,triangles);
